...
pub fn run(debug: bool) {
    ...
    let mut line_of_stones: LineOfStones = line
        .split_whitespace()
        .map(|x| x.parse::<usize>().unwrap_or_else(|_| panic!("Invalid number: {}", x)))
        .collect::<Vec<_>>()
        .into();

    let result_part1 = blink_on_line_of_stones(&mut line_of_stones, 25);
    let result_part2 = blink_on_line_of_stones(&mut line_of_stones, 75);
